...
sealed class PipelineCreatedAtColumn : DateColumn
{
	public PipelineCreatedAtColumn()
	...
	protected override Comparison<CustomListBoxItem> SortComparison => PipelineListItem.CompareByCreatedAt;

	private static bool TryGetContent(CustomListBoxItem item, out DateTimeOffset value)
	{
		if(item is PipelineListItem pipeline)
		{
			var v = pipeline.DataContext.CreatedAt;
			if(v.HasValue)
			{
				value = v.Value;
				return true;
