...
sealed class PipelineListItem : CustomListBoxItem<Pipeline>
{
	...
	public PipelineListItem(Pipeline pipeline)
		: base(pipeline)
	{
		Verify.Argument.IsNotNull(pipeline);
	}
	...
	public override ContextMenuStrip GetContextMenu(ItemContextMenuRequestEventArgs requestEventArgs)
	{
		...
		var viewTestReportItem = new ToolStripMenuItem(Resources.StrViewTestReport, null,
			(_, _) => GitterApplication.WorkingEnvironment.ViewDockService.ShowView(Guids.TestReportViewGuid, new TestReportViewModel(DataContext.Id)));
		menu.Items.Add(viewTestReportItem);
		dpiBindings.BindImage(viewTestReportItem, CommonIcons.Test);
