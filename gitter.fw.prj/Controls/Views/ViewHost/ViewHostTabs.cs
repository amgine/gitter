...
public sealed class ViewHostTabs : Control, IEnumerable<ViewHostTab>
{
	...
	internal ViewHostTabs(ViewHost viewHost, AnchorStyles side)
	{
		...
		if(viewHost.IsDocumentWell)
		{
			...
			var tabFooterHeight = Renderer.TabFooterHeight.GetValue(dpi);
	...
	{
		Verify.Argument.IsNotNull(view1);
		Verify.Argument.IsNotNull(view2);
	...
	private int HitTestArea(int x, int y)
	{
		var width = Width;
		var dpi   = Dpi.FromControl(this);
		if(x < 0 || x > width || y < 0 || y >= ViewManager.Renderer.TabHeight.GetValue(dpi))
		{
			return -1;
		}
		if(LeftButtons != null)
	...
	private int HitTestTab(int x, int y, out Rectangle bounds)
	{
		...
		if(y >= 0 && y < tabHeight)
		{
			...
			for(int i = FirstTabIndex; i < _tabs.Count; ++i)
			{
				...
				if(x >= cx && x < cx2)
				{
					bounds = new Rectangle(cx, 0, length, tabHeight);
					return i;
				}
				cx = cx2;
				if(space <= 0) break;
			...
			case ViewButtonType.TabsScrollMenu:
				if(_tabs.Count != 0)
				{
					...
					foreach(var tab in _tabs)
					{
						...
						dpiBindings.BindImage(item, tab.ImageProvider);
						menu.Items.Add(item);
					}
					Utility.MarkDropDownForAutoDispose(menu);
					var dpi  = Dpi.FromControl(this);
					var conv = DpiConverter.FromDefaultTo(dpi);
					var viewButtonSize = Renderer.ViewButtonSize.GetValue(dpi);
					menu.Show(this,
						Width - viewButtonSize - conv.ConvertX(1),
						viewButtonSize + conv.ConvertY(4));
				}
				break;
		}
	}
				...
				if(tabId != -1)
				{
					...
					else
					{
						_readyToFloat = e.Button == MouseButtons.Left &&
							_tabs[tabId].Buttons.PressedButton is null;
					}
				}
				else
				{
	...
	protected override void OnDpiChangedAfterParent(EventArgs e)
	{
		if(LeftButtons is not null)
		{
			LeftButtons.Height = Height;
		}
		foreach(var tab in _tabs)
		{
			tab.Buttons.Height = Height;
			tab.InvalidateLayout();
		}
		if(RightButtons is not null)
		{
			RightButtons.Height = Height;
		}
		Invalidate();
