...
/// <summary>
/// Collection of auto-hiding <see cref="ViewHost"/>'s on one side of <see cref="Controls.DockPanel"/>,
/// represented as tabs.
/// </summary>
[ToolboxItem(false)]
[DesignerCategory("")]
public sealed class DockPanelSide : Control, IEnumerable<ViewHost>
{
	#region Data
	...
	/// <summary>Initializes a new instance of the <see cref="DockPanelSide"/> class.</summary>
	/// <param name="dockPanel">Host grid.</param>
	/// <param name="side">Align side.</param>
	public DockPanelSide(DockPanel dockPanel, AnchorStyles side)
	{
	...
	public override Size GetPreferredSize(Size proposedSize)
		=> Orientation switch
		{
			Orientation.Horizontal => new Size(_size, Renderer.SideTabSize.GetValue(Dpi.FromControl(this)).Height),
			Orientation.Vertical   => new Size(Renderer.SideTabSize.GetValue(Dpi.FromControl(this)).Width, _size),
	...
	/// <summary>Host <see cref="Controls.DockPanel"/>.</summary>
	...
	/// <summary>Adds <see cref="ViewHost"/> to this <see cref="DockPanelSide"/>.</summary>
	...
	public void AddHost(ViewHost viewHost)
	{
		...
		using(var graphics = CreateGraphics())
		{
			if(viewHost.ViewsCount != 0)
			{
				...
				{
					var tab = new DockPanelSideTab(this, viewHost, view);
		...
		switch(Orientation)
		{
			case Orientation.Vertical:
				{
					var space = DockPanel.VerticalClientSpace;
			...
			case Orientation.Horizontal:
				{
					var space = DockPanel.HorizontalClientSpace;
	...
	private void OnViewAdded(object sender, ViewEventArgs e)
	{
		...
		var tab = new DockPanelSideTab(this, host, e.View);
		tab.ResetLength();
		var size = tab.Length;
		_tabs.Insert(i, tab);
		switch(_orientation)
	...
	/// <summary>Gets the count of <see cref="DockPanelSideTab"/>s.</summary>
	...
	private Rectangle GetTabBounds(int index)
	{
		...
		{
			case Orientation.Horizontal:
				bounds.Height = Renderer.SideTabSize.GetValue(dpi).Height;
				break;
			case Orientation.Vertical:
				bounds.Width = Renderer.SideTabSize.GetValue(dpi).Width;
				break;
			default:
				throw new ApplicationException();
		}
	...
	private void SpawnPanel(DockPanelSideTab tab)
	{
		SpawnPanel(tab, false);
	}

	private void SpawnPanel(DockPanelSideTab tab, bool activate)
	{
		var dpi = Dpi.FromControl(this);
		...
		else
		{
			...
			switch(_side)
			{
				case AnchorStyles.Left:
					{
						...
						bounds = new Rectangle(Renderer.SideTabSize.GetValue(dpi).Width, Top, w, DockPanel.VerticalClientSpace);
				...
				case AnchorStyles.Top:
					{
						...
						bounds = new Rectangle(Left, Renderer.SideTabSize.GetValue(dpi).Height, DockPanel.HorizontalClientSpace, h);
				...
				case AnchorStyles.Right:
					{
						var w = _visibleHost.Width;
						bounds = new Rectangle(Left - w, Top, w, DockPanel.VerticalClientSpace);
				...
				case AnchorStyles.Bottom:
					{
						var h = _visibleHost.Height;
						bounds = new Rectangle(Left, Top - h, DockPanel.HorizontalClientSpace, h);
			...
			{
				DockPanel.LeftSide?.DespawnPanel();
			}
			if(_side != AnchorStyles.Top)
			{
				DockPanel.TopSide?.DespawnPanel();
			}
			if(_side != AnchorStyles.Right)
			{
				DockPanel.RightSide?.DespawnPanel();
			}
			if(_side != AnchorStyles.Bottom)
			{
				DockPanel.BottomSide?.DespawnPanel();
			}
			_visibleHost.SetActiveView(tab.View);
			_visibleHost.Bounds = bounds;
			_visibleHost.Anchor = anchor;
			_visibleHost.Parent = DockPanel;
	...
	private void DespawnPanel()
	{
		if(_visibleHost != null)
		{
			if(_visibleHost.Parent == DockPanel)
				_visibleHost.Parent = null;
			_visibleHost = null;
