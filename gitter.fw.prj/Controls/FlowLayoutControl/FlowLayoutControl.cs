...
public class FlowLayoutControl : ScrollableControl
{
	...
	{
		Verify.Argument.IsNotNull(panel);
	...
	private Size GetPanelSize(Graphics graphics, FlowPanel panel)
	{
		...
		{
			size = panel.Measure(
				new FlowPanelMeasureEventArgs(graphics, Dpi.FromControl(this), ClientArea.Width));
	...
	private Size GetPanelSize(Graphics graphics, FlowPanel panel, int width)
	{
		...
		{
			size = panel.Measure(
				new FlowPanelMeasureEventArgs(graphics, Dpi.FromControl(this), width));
			_sizes.Add(panel, size);
		}
		return size;
	}
	...
	{
		if(_mouseDownPanel is not null && _mouseDownPanel.FlowControl == this)
		{
	...
	{
		if(_mouseDownPanel is not null)
		{
		...
		for(int i = 0; i < Panels.Count; ++i)
		{
			...
			if(maxY >= ContentArea.Y)
			{
				...
				if(maxY >= clip.Y && y < clippingEdge)
				{
					...
					if(Rectangle.Intersect(clip, extBounds) is { Width: > 0, Height: > 0 } panelClip)
					{
						graphics.SetClip(panelClip);
						panel.Paint(new FlowPanelPaintEventArgs(graphics, dpi, panelClip, bounds));
					}
				}
			}
			y = maxY;
