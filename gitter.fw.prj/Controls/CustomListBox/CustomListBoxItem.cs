...
public abstract class CustomListBoxItem : CustomListBoxHostedItem
{
	...
	protected override int OnHitTest(int x, int y)
	{
		...
		int start = level * levelMargin + ListBoxConstants.RootMargin;
		int end;
		...
		if(listBox.ShowTreeLines && (level != 0 || listBox.ShowRootTreeLines))
		{
			...
			if(level != 0 || listBox.ShowRootTreeLines)
			{
				start += ListBoxConstants.SpaceBeforePlusMinus.GetValue(dpi);
				end = start + ListBoxConstants.PlusMinusImageWidth.GetValue(dpi);
				if(x >= start && x < end)
				{
...
public abstract class CustomListBoxItem<T> : CustomListBoxItem, IDataContextProvider<T>
{
