...
{
	private readonly RemotesListBox _lstRemotes;
	private readonly RemotesToolbar _toolbar;
	private readonly ISearchToolBarController _searchToolbar;

	public RemotesView(GuiProvider gui, IFactory<RemotesListBox> remoteListBoxFactory)
		: base(Guids.RemotesViewGuid, gui)
	{
		Verify.Argument.IsNotNull(remoteListBoxFactory);

		SuspendLayout();
		_lstRemotes = remoteListBoxFactory.Create();
		_lstRemotes.BorderStyle = BorderStyle.None;
		_lstRemotes.Dock        = DockStyle.Fill;
		_lstRemotes.Margin      = Padding.Empty;
		_lstRemotes.Name        = nameof(_lstRemotes);
		_lstRemotes.TabIndex    = 0;
		_lstRemotes.Parent      = this;
		Name = nameof(RemotesView);
		ResumeLayout(false);
		PerformLayout();
