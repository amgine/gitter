...
internal sealed class HistoryToolbar : ToolStrip
{
	...
	public HistoryToolbar(HistoryView view)
	{
		...
		Items.AddRange(
			[
				...
				// right-aligned
				_btnShowDetails = new ToolStripButton(Resources.StrAutoShowDiff, null, OnShowDetailsButtonClick)
					{
		...
		_dpiBindings.BindImage(_btnFilter,      Icons.Filter);
		_dpiBindings.BindImage(_btnShowDetails, Icons.Diff);
	...
	private void UpdateLimitButtonText()
	{
		...
		{
			if(item.Tag is int maxCount && maxCount == _view.LogOptions.MaxCount)
			{
	...
	public ToolStripButton ShowDiffButton => _btnShowDetails;
}
