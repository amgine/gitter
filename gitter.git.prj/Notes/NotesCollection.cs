...
public sealed class NotesCollection : GitObject, IEnumerable<Note>
{
	...
	public bool TryGetNote(string name, [MaybeNullWhen(returnValue: false)] out Note note)
	{
	...
	public async Task RefreshAsync()
	{
		var notes = await Repository.Accessor.QueryNotes
			.InvokeAsync(new QueryNotesRequest())
			.ConfigureAwait(continueOnCapturedContext: false);
		Refresh(notes);
	}

	internal void Append(IRevisionPointer revision, string message)
	{
		Repository.Accessor.AppendNote.Invoke(
			new AppendNoteRequest()
			{
				Revision = revision.Pointer,
				Message  = MessageSpecification.FromText(message),
			});
	}
