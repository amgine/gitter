namespace gitter.Git
{
	...
	public abstract class TreeItem : GitLifeTimeNamedObject
	{
		...
		public Diff GetDiff()
		{
			switch(_stagedStatus)
			{
				case StagedStatus.Staged:
					return Repository.Status.GetStagedDiff(RelativePath);
				case StagedStatus.Unstaged:
					return Repository.Status.GetUnstagedDiff(RelativePath);
			}
			return null;
		}
		...
		public IAsyncFunc<Diff> GetDiffAsync()
		{
			...
			{
				case StagedStatus.Staged:
					return Repository.Status.GetStagedDiffAsync(RelativePath);
				case StagedStatus.Unstaged:
					return Repository.Status.GetUnstagedDiffAsync(RelativePath);
			}
			return null;
		}
