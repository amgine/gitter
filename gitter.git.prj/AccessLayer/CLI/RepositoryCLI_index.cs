namespace gitter.Git.AccessLayer
{
	...
	internal sealed partial class RepositoryCLI : IIndexAccessor
	{
		...
		private static void InsertCleanFilesParameters(CleanFilesParameters parameters, IList<CommandArgument> args)
		{
			if(parameters.Force)
			{
			...
			if(parameters.ExcludePatterns != null && GitFeatures.CleanExcludeOption.IsAvailable)
			{
		...
		public StatusData QueryStatus(QueryStatusParameters parameters)
		{
			...
			if(RepositoryProvider.GitVersion >= SubmodulesStatusArgVersion)
			{
