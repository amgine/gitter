...
sealed class TagChangedNotification(string name) : RepositoryChangedNotification
{
	...
	public override bool Apply(Repository repository)
	{
		...
		else
		{
			...
			lock(refs.SyncRoot)
			{
				...
				{
					if(tag.Revision is null || tag.Revision.Hash != tagInformation.Hash)
					{
						refs.NotifyRemoved(Name);
						refs.NotifyCreated(tagInformation);
					}
